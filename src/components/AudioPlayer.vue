<template>
    <div class="flex items-center relative bg-neutral-800 shadow-black-lg">
        <button
        ...
        <div
            class="w-11 px-2 uppercase font-medium font-mono text-neutral-400 text-xs flex flex-col items-center"
        >
            <span>{{ duration }}</span>
            <span class="text-white">{{ progress }}</span>
        </div>
        <img
            v-if="$store.state.trackPlaying.image && !sidebar"
            :src="image"
            class="h-14 w-14"
            alt=""
        />
        <div
            v-if="!$store.state.trackPlaying.image && artist && !sidebar"
            class="h-14 w-14 bg-neutral-800 flex justify-center items-center font-bold text-neutral-400"
        >
            <span>?</span>
        </div>
        <div
            ...
            class="h-14 w-14 bg-neutral-800 flex justify-center items-center text-neutral-400 text-xxs"
        ></div>
    </div>
    <div v-if="!sidebar" class="font-medium text-neutral-400">
        <p class="uppercase">now playing</p>
        <p
            class="text-neutral-100 text-xs whitespace-nowrap overflow-hidden overflow-ellipsis"
...
.wavesurfer-marker svg {
    ...
    polygon {
        ...
        stroke: transparent;
        // fill: #131313;
        @apply fill-current text-neutral-400;
