<template>
    <li v-if="folder.$.TYPE == 'FOLDER'" class="node-tree">
        <div
            class="flex py-1 px-2 space-x-2 cursor-pointer hover:bg-primary-600 hover:text-white"
            @click="toggleFolder"
        >
            <svg-icon
                v-if="!open"
            ...
            ></svg-icon>
            <span>{{
                folder.$.NAME == '$ROOT' ? 'Playlists' : folder.$.NAME
            }}</span>
        </div>
        <ul
            v-if="
                folder.SUBNODES[0].NODE &&
                folder.SUBNODES[0].NODE.length &&
                open
            "
            class="pl-3"
        >
    ...
    <li v-if="folder.$.TYPE == 'PLAYLIST'" class="playlist">
        <div
            class="flex py-1 px-2 space-x-2 cursor-pointer hover:bg-primary-600 hover:text-white"
            :class="{
    ...
    methods: {
        ...
        openPlaylist(list) {
            ...
            for (const playlistTrack of list.ENTRY) {
                let filename =
                    playlistTrack.PRIMARYKEY[0].$.KEY.split('/:').pop();
                let index = parseInt(
                    this.$store.state.filenameToIndex[filename]
                );
