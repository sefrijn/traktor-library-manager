    ...
    methods: {
        ...
        onBodyScroll: throttle(20, function (event) {
            ...
            if (
                ...
                // Calculate ratio
                let h =
                    this.gridApi.gridBodyCon.eBodyViewport.children[1]
                        .clientHeight;
                let newScrollRatio =
            ...
            if (this.scrollSource == 'visualbrowser') {
                ...
                h =
                    this.gridApi.gridBodyCon.eBodyViewport.children[1]
                        .clientHeight;
                this.gridApi.gridBodyCon.eBodyViewport.scrollTop =
                    newScrollRatio * h;
            }
        }),
        ...
        onRowDragEnd(event) {
            ...
            if (this.activePlaylist) {
                let entries =
                    this.$store.getters.playlistEntries[this.activePlaylist];
            ...
            dropZoneParams = {
                ...
                onDragStop: (params) => {
                    ...
                    if (id.includes('playlist')) {
                        ...
                        this.$store.commit('setLibraryPlaylist');
                        // Save to XML file
                        let libraryObj = cloneDeep(
                            this.$store.getters.libraryFull
                        );
                        window.ipcRenderer.send('buildXML', [
                            libraryObj,
                            localStorage.pathToLibrary,
        ...
        onCellClicked(params) {
            if (params.data.index != this.$store.state.trackPlaying.index) {
                let clipboardVal =
                    params.data.artist + ' - ' + params.data.title;
        ...
        save(params) {
            ...
            // Remixer
            this.$store.commit('setLibraryValue', {
                path: nmlCollection + i + '.INFO.0.$.REMIXER',
                value: params.data.remixer,
            });
            // Title
            this.$store.commit('setLibraryValue', {
            ...
            // Mix
            this.$store.commit('setLibraryValue', {
                path: nmlCollection + i + '.INFO.0.$.MIX',
                value: params.data.mix,
            });
            // Genre
            this.$store.commit('setLibraryValue', {
            ...
            // Release Date
            this.$store.commit('setLibraryValue', {
                path: nmlCollection + i + '.INFO.0.$.RELEASE_DATE',
                value: params.data.release_date,
            });
            // Catalog Number
            this.$store.commit('setLibraryValue', {
                path: nmlCollection + i + '.INFO.0.$.CATALOG_NO',
                value: params.data.catalog_no,
            });
