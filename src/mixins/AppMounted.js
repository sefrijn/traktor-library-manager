    ...
    mounted() {
        ...
        window.ipcRenderer.receive('coverArtList', (message) => {
            ...
            collection.forEach((track, index) => {
                let filename = track['LOCATION'][0]['$']['FILE'].replace(
                    /\/\//g,
                    ':'
                );
                ...
                collectionFiltered[index] = {
                    ['index']: index,
                    ['artist']: track['$']['ARTIST'],
                    ["remixer"]: track["INFO"][0]["$"]["REMIXER"],
                    ['title']: track['$']['TITLE'],
                    ["mix"]: track["INFO"][0]["$"]["MIX"],
                    ['length']: this.fancyTimeFormat(track['INFO'][0]['$']['PLAYTIME'], true),
                    ['genre']: genre,
                    ['comment_1']: tags1_str,
                    ['comment_2']: tags2_str,
                    ['rating']: track['INFO'][0]['$']['RANKING'] ? track['INFO'][0]['$']['RANKING'] / 51 : 0,
                    ['color_code']: track['INFO'][0]['$']['COLOR'],
                    ['musical_key']: typeof track['MUSICAL_KEY'] === 'undefined' ? 0 : track['MUSICAL_KEY'][0]['$']['VALUE'],
                    ["key"]: typeof track['KEY'] === 'undefined' ? '' : track['INFO'][0]['$']['KEY'],
                    ['bpm']: typeof track['TEMPO'] === 'undefined' ? '' : Math.round(track['TEMPO'][0]['$']['BPM'] * 100) / 100,
                    ['import_date']: track['INFO'][0]['$']['IMPORT_DATE'],
                    ['release_date']: track['INFO'][0]['$']['RELEASE_DATE'],
                    ['play_count']: track['INFO'][0]['$']['PLAYCOUNT'],
                    ['path']: track['LOCATION'][0]['$']['DIR'].replace(/:/g, ''),
                    ['image']: message[index].file == null ? null : `${filename.substring(0, filename.lastIndexOf('.'))}.jpeg`,
                    ['filename']: filename,
                    ['cue_points']: track['CUE_V2'],
                    ["catalog_no"]: track["INFO"][0]["$"]["CATALOG_NO"],
                };
        ...
        window.ipcRenderer.receive('parseXML', (xmlAsJS) => {
            ...
            collection.forEach((track, index) => {
                paths[index] = {
                    path: track['LOCATION'][0]['$']['DIR'].replace(/:/g, ''),
                    file: track['LOCATION'][0]['$']['FILE'].replace(
                        /\/\//g,
                        ':'
                    ),
                };
